...
namespace Chroma
{
  ...
  /*! @ingroup gaugeacts */
  void read(XMLReader &xml, const std::string &path, AnisoSpectrumGaugeAct2Params &param);

  /*! @ingroup gaugeacts */
  void write(XMLWriter &xml, const std::string &path, const AnisoSpectrumGaugeAct2Params &param);
  ...
  public:
    ...
    AnisoSpectrumGaugeAct2(Handle<CreateGaugeState<P, Q>> cgs_,
                           const AnisoSpectrumGaugeAct2Params &p) : param(p) { init(cgs_); }
    ...
    void update_coeff(multi1d<LatticeColorMatrix>& u)
    {
      ...
      param.rect_c_t_2 = -param.beta / (Real(12) * u_s_4 * u_t_2) * param.aniso.xi_0;
      if (param.aniso.anisoP)
    ...
    */
                ...
                int mu, int cb) const
    {
      ...
      for (int nu = 0; nu < Nd; ++nu)
      {
        ...
        // 1mu 1nu up staple
        result[rb[cb]] += coeff_p * shift(u[nu], FORWARD, mu) * adj(shift(u[mu], FORWARD, nu)) * adj(u[nu]);

        // 1mu 1nu down staple
        result[rb[cb]] += coeff_p * adj(shift(shift(u[nu], FORWARD, mu), BACKWARD, nu)) * adj(shift(u[mu], BACKWARD, nu)) * shift(u[nu], BACKWARD, nu);

        if(nu == param.aniso.t_dir || ifspace)
        {
        // 2 mu * 1nu up staple
        result[rb[cb]] += coeff_r * shift(u[mu], FORWARD, mu) * shift(shift(u[nu], FORWARD, mu), FORWARD, mu) * adj(shift(shift(u[mu], FORWARD, mu), FORWARD, nu)) * adj(shift(u[mu], FORWARD, nu)) * adj(u[nu]);

        // 2 mu * 1nu down staple
        result[rb[cb]] += coeff_r * shift(u[mu], FORWARD, mu) * adj(shift(shift(shift(u[nu], FORWARD, mu), FORWARD, mu), BACKWARD, nu)) * adj(shift(shift(u[mu], FORWARD, mu), BACKWARD, nu)) * adj(shift(u[mu], BACKWARD, nu)) * shift(u[nu], BACKWARD, nu);
        }

        if(mu == param.aniso.t_dir || ifspace)
        {
        // 2 nu * 1mu up staple
        result[rb[cb]] += coeff_r * shift(u[nu], FORWARD, mu) * shift(shift(u[nu], FORWARD, mu), FORWARD, nu) * adj(shift(shift(u[mu], FORWARD, nu), FORWARD, nu)) * adj(shift(u[nu], FORWARD, nu)) * adj(u[nu]);

        // 2 nu * 1mu down staple
        result[rb[cb]] += coeff_r * adj(shift(shift(u[nu], FORWARD, mu), BACKWARD, nu)) * adj(shift(shift(shift(u[nu], FORWARD, mu), BACKWARD, nu), BACKWARD, nu)) * adj(shift(shift(u[mu], BACKWARD, nu), BACKWARD, nu)) * shift(shift(u[nu], BACKWARD, nu), BACKWARD, nu) * shift(u[nu], BACKWARD, nu);
        }
      }
  ...
  private:
    ...
    Handle<PlaqGaugeAct> plaq; /*!< Hold spatial plaquettes separated in time type gaugeact */
  };
