...
double effective_mass(int nt_h, int it, double a, double b, int flag = 0,
                      double x1 = 0.0, double x2 = 10.0);

int name_c2i(char *name);

struct int_rand {
  int virtual i_rand(int max) const { return rand() % max; }
};
...
public:
  ...
  one_dim &fdim(int name) { return dim[find_dim(name)]; }
  int nind(int name) {
    int ind = find_dim(name);
    if (ind >= 0)
      return dim[ind].n_indices;
    else
      return 1;
  }
...
class general_data_base : public datatype {
  ...
  general_data_base *set_pointer(general_data_base &res,
...
public:
  ...
  general_data_base(filetype &type0, const char *namex, double *data0) {
    ...
    type = type0;
    initialize();
    memcpy(data, data0, size * sizeof(double));
  }
  ...
  template <typename type>
  void gather_file_bin(const char *name, std::vector<type> &list,
                       int bin_size = 1) {
    free_data();
  ...
  int pick(general_data_base &res, int name, std::vector<int> list,
           int flag = 0) {
  ...
  int bootstrap(general_data_base &res, int nboot,
                const int_rand &r = int_rand());
};
    ...
    for (i = 0; i < nfixdims; i++) {
      ...
      for (j = 0; j < rawdata.ndim; j++)
        if (ty.head.dimensions[i].type == rawdata.dim[j].type) {
          ...
          sort_indices(ty.head.dimensions[i].indices,
                       ty.head.dimensions[i].n_indices);
          break;
        }
      if (j == rawdata.ndim) {
