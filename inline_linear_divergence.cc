...
namespace Chroma
{
  ...
  //! Propagator input
  void read(XMLReader &xml, const std::string &path, InlineLinearDivergenceParams::NamedObject_t &input)
  {
  ...
  //! Propagator output
  void write(XMLWriter &xml, const std::string &path, const InlineLinearDivergenceParams::NamedObject_t &input)
  {
    push(xml, path);
  ...
  InlineLinearDivergenceParams::InlineLinearDivergenceParams(XMLReader &xml_in, const std::string &path)
  {
    try
    {
      ...
      if (grid.size() != 3)
      {
        QDPIO::cerr << InlineLinearDivergenceEnv::name << ": wrong size of grid array: expected length=" << 3
                    << std::endl;
        QDP_abort(1);
      }
      ...
      {
        QDPIO::cerr << InlineLinearDivergenceEnv::name << ": wrong size of link_length array: expected length=" << 3
                    << std::endl;
        QDP_abort(1);
      }
      ...
      if (HL_ratio <= 0)
      {
        QDPIO::cerr << InlineLinearDivergenceEnv::name << ": set HL_ratio<=0 is pointless, should be larger than 0."
                    << std::endl;
        QDP_abort(1);
      }
      if (nsets % HL_ratio != 0)
      {
        QDPIO::cerr << InlineLinearDivergenceEnv::name << ": HL_ratio (" << HL_ratio << ") should be a factor of nsets (" << nsets << ")."
                    << std::endl;
        QDP_abort(1);
      }
  ...
  void InlineLinearDivergenceParams::writeXML(XMLWriter &xml_out, const std::string &path)
  {
    push(xml_out, path);
  ...
  // Real work done here
  void InlineLinearDivergence::func(unsigned long update_no,
                                    XMLWriter &xml_out)
  {
    START_CODE();

    QDPIO::cout << InlineLinearDivergenceEnv::name << ": loop calculation" << std::endl;
    ...
    {
      QDPIO::cerr << InlineLinearDivergenceEnv::name << ": caught dynamic cast error"
                  << std::endl;
      QDP_abort(1);
    }
    catch (const std::string &e)
    {
      QDPIO::cerr << InlineLinearDivergenceEnv::name << ": std::map call failed: " << e
                  << std::endl;
      QDP_abort(1);
    }
    ...
    for (int idr = 0; idr < 3; idr++)
      offset[idr] = (params.grid[idr] > 0) ? params.grid[idr] : (Layout::lattSize()[idr]);
    offset[3] = 128;
    ...
    QDPIO::cout << InlineLinearDivergenceEnv::name << ": initialized " << std::endl;
...
//        c, LatticeComplex(zero));
    ...
    if (!success)
    {
      ...
      {
        QDPIO::cout << InlineLinearDivergenceEnv::name << ": caught exception around quarkprop: " << e << std::endl;
      }
    }
    ...
    tmpu1 = where(
      ...
      for (int iop = 0; iop < nx; iop++)
      {
        if (icount == nx)
        {
          quark_loop[ig + icount * 16] = LatticeComplex(trace(F_mu * c2_dagger * Gamma(ig) * adj(F_mu) * Gamma(ig)));
        }
        else
        {
          quark_loop[ig + icount * 16] = LatticeComplex(trace(F_mu * Gamma(ig) * adj(F) * Gamma(ig)));
        }
        icount++;
    ...
    for (int ig = 0; ig < 16; ig++)
      quark_loop[ig + icount * 16] = LatticeComplex(trace(Gamma(15) * adj(F) * Gamma(15) * Gamma(ig) * F * Gamma(ig)));
    icount++;
    ...
    for (int mu = 0; mu < 4; mu++)
    {
      ...
      quark_loop[mu - 1 + icount * 16] = 0.5 * LatticeComplex(trace(Gamma(15) * adj(F) * Gamma(15) *
                                                                    (F_plus - F_minus) * Gamma(gamma_index)));
      gamma_index = gamma_index * 2;
    }
    icount++;

    QDPIO::cout << InlineLinearDivergenceEnv::name << "quark loop done" << std::endl;
    ...
    for (int is = 0; is < op_size; is++)
    {
      for (int ig = 0; ig < 16; ig++)
      {
        ...
        if (Layout::primaryNode())
          ...
          {
            result.data[(is * 16 + ig) * 2 * nt + 2 * t] -= toDouble(hsum[t].elem().elem().elem().real());
            result.data[(is * 16 + ig) * 2 * nt + 2 * t + 1] -= toDouble(hsum[t].elem().elem().elem().imag());
          }
      }
    }
    ...
    snoop.stop();
    QDPIO::cout << InlineLinearDivergenceEnv::name << ": total time = "
                << snoop.getTimeInSeconds()
                << " secs" << std::endl;

    QDPIO::cout << InlineLinearDivergenceEnv::name << ": ran successfully" << std::endl;
