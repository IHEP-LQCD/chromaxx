...
namespace Chroma
{
  ...
  { 
    ...
    const std::string name = "ANISO_SPECTRUM_GAUGEACT2";
  ...
  AnisoSpectrumGaugeAct2Params ::AnisoSpectrumGaugeAct2Params (XMLReader& xml_in, const std::string& path) 
  {
  ...
  void read(XMLReader& xml, const std::string& path, AnisoSpectrumGaugeAct2Params & p) 
  {
    AnisoSpectrumGaugeAct2Params tmp(xml, path);
    p=tmp;
  }
  ...
  // Private initializer
  void
  AnisoSpectrumGaugeAct2::init(Handle< CreateGaugeState<P,Q> > cgs)
  {
    START_CODE();
