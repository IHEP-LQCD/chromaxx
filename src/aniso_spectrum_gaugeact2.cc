...
bool registerAll() {
  ...
  if (!registered) {
    success &=
        TheGaugeActFactory::Instance().registerObject(name, createGaugeAct);
    registered = true;
  }
  return success;
}
...
void read(XMLReader &xml, const std::string &path,
          AnisoSpectrumGaugeAct2Params &p) {
...
void AnisoSpectrumGaugeAct2::init(Handle<CreateGaugeState<P, Q>> cgs) {
  ...
  rect = new RectGaugeAct(cgs, rect_c_s, rect_c_t_1, rect_c_t_2,
                          no_temporal_2link, param.aniso);
