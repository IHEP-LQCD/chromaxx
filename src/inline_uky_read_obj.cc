...
bool registerAll() {
  ...
  if (!registered) {
    // Datatype reader
    success &= UKYReadObjCallMapEnv::registerAll();
...
void InlineMeas::operator()(unsigned long update_no, XMLWriter &xml_out) {
  ...
  push(xml_out, "UKY_read_named_obj");
  ...
  QDPIO::cout << "Attempt to read object name = " << params.named_obj.object_id
              << std::endl;
  QDPIO::cout << "Attempt to read file name = " << params.file.file_name
              << std::endl;
  ...
  try {
    ...
    //      // Read the object
    //      TheUKYReadObjFuncMap::Instance().dump();

    // Read the object
    swatch.start();
    UKYReadObjCallMapEnv::TheUKYReadObjFuncMap::Instance().callFunction(
