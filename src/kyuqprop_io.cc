...
*/
  ...
  for (int src_spin = 0; src_spin < Ns; ++src_spin)
    for (int src_color = 0; src_color < Nc; ++src_color) {
      ...
      for (int snk_spin = 0; snk_spin < Ns; ++snk_spin) {
        for (int snk_color = 0; snk_color < Nc; ++snk_color) {
          pokeColor(cv, cmplx(re(snk_color, snk_spin), im(snk_color, snk_spin)),
                    snk_color);
        }
  ...
  for (int src_dirac = 0; src_dirac < Nd; ++src_dirac)
    for (int src_color = 0; src_color < Nc; ++src_color)
      FermToProp(LatticeFermionD(source(src_color, src_dirac)), q_old,
                 src_color, src_dirac);
...
#if 1
  {
    ...
    LatticeComplex ps_rho =
        trace(adj(gamma_5 * q_old * gamma_5) * gamma_1 * q_old * gamma_1);

    LatticeComplex dr_rho =
        trace(adj(Gamma(15) * q * Gamma(15)) * Gamma(1) * q * Gamma(1));

    QDPIO::cout << "readKYUQprop: norm2(ps_rho) = " << RealD(norm2(ps_rho))
                << std::endl;
    QDPIO::cout << "readKYUQprop: norm2(dr_rho) = " << RealD(norm2(dr_rho))
                << std::endl;
    QDPIO::cout << "readKYUQprop: norm2(ps_rho - dr_rho) = "
                << RealD(norm2(ps_rho - dr_rho)) << std::endl;
    QDPIO::cout << "readKYUQprop: norm2(ps_rho + dr_rho) = "
                << RealD(norm2(ps_rho + dr_rho)) << std::endl;
