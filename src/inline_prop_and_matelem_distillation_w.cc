...
public:
  //! Constructor
  SubEigenMap(MODS_t &eigen_source_, int decay_dir, bool zero_colorvecs)
      : eigen_source(eigen_source_), time_slice_set(decay_dir),
...
const SubLatticeColorVectorF &SubEigenMap::getVec(int t_source,
  ...
  // If item does not exist, read from original std::map and put in memory
  // std::map
  if (!sub_eigen.exist(src_key)) {
    QDPIO::cout << __func__ << ": on t_source= " << t_source
...
std::vector<bool> getActiveTSlices(int t_source, int Nt_forward,
                                   int Nt_backward) {
...
//----------------------------------------------------------------------------
//! Get sink keys
std::list<KeyPropElementalOperator_t> getSnkKeys(int t_source, int spin_source,
                                                 int Nt_forward,
                                                 int Nt_backward,
                                                 const std::string mass) {
  std::list<KeyPropElementalOperator_t> keys;

  std::vector<bool> active_t_slices =
      getActiveTSlices(t_source, Nt_forward, Nt_backward);
...
void InlineMeas::func(unsigned long update_no, XMLWriter &xml_out) {
  ...
  //
  QDPIO::cout << "Snarf the source from a std::map object disk file"
              << std::endl;
  ...
  if (!params.param.contract.zero_colorvecs) {
    try {
      // Open
      QDPIO::cout << "Open file= " << params.named_obj.colorvec_files[0]
                  << std::endl;
  ...
  SubEigenMap sub_eigen_map(eigen_source, decay_dir,
                            params.param.contract.zero_colorvecs);
  ...
  try {
    ...
    for (int tt = 0; tt < t_sources.size(); ++tt) {
      ...
      for (int spin_source = 0; spin_source < Ns; ++spin_source) {
        ...
        // These are the common parts of a perambulator that are needed for this
        // time source
        std::list<KeyPropElementalOperator_t> snk_keys(
            getSnkKeys(t_source, spin_source, params.param.contract.Nt_forward,
        ...
        if (1) {
          ...
          for (int colorvec_src = 0; colorvec_src < num_vecs; ++colorvec_src) {
            ...
            // Use the same colorstd::vector source. No spin dilution will be
            // used.
            //
            ...
            if (!params.param.contract.zero_colorvecs) {
              ...
              for (int nn = 1; nn <= params.param.contract.num_tries; ++nn) {
                ...
                if (toDouble(res.resid) > 1.0e-3) {
                  QDPIO::cerr << name
                              << ": have a resid > 1.0e-3. That is unacceptable"
                              << std::endl;
                  QDP_abort(1);
                }
            ...
            snarss1.stop();
            QDPIO::cout << "Time to compute prop for spin_source= "
                        << spin_source << "  colorvec_src= " << colorvec_src
...
// Loop over time
            ...
            for (int t_slice = 0; t_slice < Lt; ++t_slice) {
              ...
              for (std::list<KeyPropElementalOperator_t>::const_iterator key =
                       snk_keys.begin();
                   key != snk_keys.end(); ++key) {
                if (key->t_slice != t_slice) {
                  continue;
                }

                // Loop over the sink colorvec, form the innerproduct and the
                // resulting perambulator
                for (int colorvec_sink = 0; colorvec_sink < num_vecs;
                  ...
                  if ((key->t_slice - key->t_source) < 0) {
                    peram_array[(key->t_slice) + Lt - (key->t_source)][spin_snk]
            ...
            if (params.param.contract.fuse_timeloop) {
              ...
              for (int spin_snk = 0; spin_snk < Ns; ++spin_snk) {
                ...
                for (std::list<KeyPropElementalOperator_t>::const_iterator key =
                         snk_keys.begin();
                     key != snk_keys.end(); ++key) {
                  if (key->spin_snk != spin_snk) {
                    continue;
                  }
                  count += num_vecs;
                }

                // QDPIO::cout << "spin_snk = " << spin_snk << ", count = " <<
                // count << "\n";
                ...
                for (std::list<KeyPropElementalOperator_t>::const_iterator key =
                  ...
                  for (int i = 0; i < num_vecs; ++i) {
                    vec_ptr[run_count] = const_cast<SubLatticeColorVectorF *>(
                        &sub_eigen_map.getVec(key->t_slice, i));
              ...
              for (int t_slice = 0; t_slice < Lt; ++t_slice) {
                ...
                for (std::list<KeyPropElementalOperator_t>::const_iterator key =
                  ...
                  for (int i = 0; i < num_vecs; ++i) {
                    vec_ptr[i] = const_cast<SubLatticeColorVectorF *>(
                        &sub_eigen_map.getVec(t_slice, i));
                  ...
                  // Big call-out
                  //
                  multi_innerProduct(contr_ptr, vec_ptr,
                                     ferm_out(key->spin_snk));
                  for (int colorvec_sink = 0; colorvec_sink < num_vecs;
                       ++colorvec_sink) {
                    double re = toDouble(
            ...
            sniss1.stop();
            QDPIO::cout
                << "Time to compute and assemble peram for spin_source= "
                << spin_source << "  colorvec_src= " << colorvec_src
                << "  time = " << sniss1.getTimeInSeconds() << " secs"
                << " (for contraction: "
                << sniss1.getTimeInSeconds() - snarss1.getTimeInSeconds() << ")"
                << std::endl;

          } // for colorvec_src

          if (!params.param.contract.zero_colorvecs) {
            // Write out each time-slice chunk of a lattice colorvec soln to
            // disk
            QDPIO::cout << "Write perambulator for spin_source= " << spin_source
                        << "  to disk" << std::endl;
            StopWatch sniss2;
            sniss2.reset();
            sniss2.start();
            ...
            QDPIO::cout << "Time to write perambulators for spin_src= "
                        << spin_source
                        << "  time = " << sniss2.getTimeInSeconds() << " secs"
                        << std::endl;
          }
      ...
      if (Layout::primaryNode()) {
        fwrite(peram_array, sizeof(peram_array), 1, fp1);
      }
    } // for tt

    fclose(fp1);
    QDPIO::cout << "WRITE perambulator to binary file" << std::endl;
  ...
  snoop.stop();
  QDPIO::cout << name << ": total time = " << snoop.getTimeInSeconds()
              << " secs" << std::endl;
