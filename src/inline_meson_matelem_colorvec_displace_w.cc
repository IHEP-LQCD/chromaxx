...
void read(XMLReader &xml, const std::string &path,
          InlineMesonMatElemColorVecDisplaceIHEPEnv::Params::Param_t &param) {
  ...
  default:
    ...
    QDPIO::cerr << "Input parameter version " << version << " unsupported."
                << std::endl;
    QDP_abort(1);
  }
  ...
  param.link_smearing =
      readXMLGroup(paramtop, "LinkSmearing", "LinkSmearingType");
}
...
//! Read named objects
void read(
    XMLReader &xml, const std::string &path,
    InlineMesonMatElemColorVecDisplaceIHEPEnv::Params::NamedObject_t &input) {
  XMLReader inputtop(xml, path);

  read(inputtop, "gauge_id", input.gauge_id);
  read(inputtop, "colorvec_file", input.colorvec_id);
...
void write(
  ...
  write(xml, "colorvec_file", input.colorvec_id);
...
void write(XMLWriter &xml, const std::string &path,
           const InlineMesonMatElemColorVecDisplaceIHEPEnv::Params &param) {
...
const std::string name = "MESON_MATELEM_COLORVEC_DISPLACE_IHEP";
...
struct ValMesonElementalOperator_t {
  ...
  multi2d<ComplexD> op; /*!< Colorstd::vector source and sink with momentum
                           projection */
};
...
void read(XMLReader &xml, const std::string &path,
          KeyMesonElementalOperator_t &param) {
  XMLReader paramtop(xml, path);
...
void write(XMLWriter &xml, const std::string &path,
           const KeyMesonElementalOperator_t &param) {
  push(xml, path);
...
multi1d<int> normDisp(const multi1d<int> &orig) {
  ...
  multi1d<int> disp;
  multi1d<int> empty;
  multi1d<int> no_disp(1);
  no_disp[0] = 0;

  // NOTE: a no-displacement is recorded as a zero-length array
  // Convert a length one array with no displacement into a no-displacement
  // array
  if (orig.size() == 1) {
    if (orig == no_disp)
      disp = empty;
...
void InlineMeas::func(unsigned long update_no, XMLWriter &xml_out) {
  ...
  QDPIO::cout << name << ": Meson color-std::vector matrix element"
              << std::endl;
  ...
  //
  // If a list of momenta has been specified only need phases corresponding to
  // these
  //
  ...
  try {
    ...
    QDPIO::cout << "Link smearing type = " << params.param.link_smearing.id
                << std::endl;

    Handle<LinkSmearing> linkSmearing(
        TheLinkSmearingFactory::Instance().createObject(
            params.param.link_smearing.id, linktop,
  ...
  for (int l = 0; l < params.param.displacement_list.size(); ++l) {
    ...
    for (int mom_num = 0; mom_num < phases.numMom(); ++mom_num) {
      if (norm2(phases.numToMom(mom_num)) < params.param.mom2_min)
        continue;

      // The keys for the spin and displacements for this particular elemental
      // operator
      // No displacement for left colorstd::vector, only displace right
      // colorstd::vector
      ...
      for (int t = 0; t < phases.numSubsets(); ++t) {
        ...
        if ((phases.numToMom(mom_num)) == zero_mom &&
    ...
    QDPIO::cout << "Meson operator= " << l
                << "  time= " << swiss.getTimeInSeconds() << " secs"
                << std::endl;
