...
double effective_mass(int nt_h, int it, double a, double b, int flag, double x1,
                      double x2) {
  if (flag == 0)
    return log(a / b);
...
void jack(double *res, int nconf, int nd, int nc, double *res_jk) {
  ...
  for (iconf = 0; iconf < nconf; iconf++)
    for (i = 0; i < nc; i++)
      aver[i] += res[nd * iconf + i];
  for (iconf = 0; iconf < nconf; iconf++)
    for (i = 0; i < nc; i++)
      res_jk[nd * iconf + i] = (aver[i] - res[nd * iconf + i]) / (nconf - 1);
  if (nconf == 1)
    for (i = 0; i < nc; i++)
      res_jk[i] = aver[i];
  free(aver);
...
void anti_jack(double *res, int nconf, int nd, int nc, double *res_jk) {
  ...
  for (iconf = 0; iconf < nconf; iconf++)
    for (i = 0; i < nc; i++)
      res_jk[nd * iconf + i] = aver[i] - (nconf - 1) * res[nd * iconf + i];
  if (nconf == 1)
    for (i = 0; i < nc; i++)
      res_jk[i] = aver[i];
  free(aver);
...
void jacksig(double *res, int nconf, int nd, int nc, double *aver,
             double *sig) {
  int i, iconf;
  ...
  for (iconf = 0; iconf < nconf; iconf++)
    for (i = 0; i < nc; i++)
      sig[i] += pow(aver[i] - res[nd * iconf + i], 2);
  for (i = 0; i < nc; i++)
    sig[i] = sqrt(sig[i] * (nconf - 1) / (nconf));
}

int datatype::name_c2i(char *name) {
  if (!strcmp(name, "dim_complex"))
    return dim_complex;
  if (!strcmp(name, "dim_mass"))
    return dim_mass;
  if (!strcmp(name, "dim_smear"))
    return dim_smear;
  if (!strcmp(name, "dim_conf"))
    return dim_conf;
  if (!strcmp(name, "dim_jackknife"))
    return dim_jackknife;
  if (!strcmp(name, "dim_operator"))
    return dim_operator;
  if (!strcmp(name, "dim_t"))
    return dim_t;
  if (!strcmp(name, "dim_mass2"))
    return dim_mass2;
  if (!strcmp(name, "dim_t2"))
    return dim_t2;
  if (!strcmp(name, "dim_channel"))
    return dim_channel;
  if (!strcmp(name, "dim_momentum"))
    return dim_momentum;
  if (!strcmp(name, "dim_param"))
    return dim_momentum;
  if (!strcmp(name, "dim_bootstrap"))
    return dim_bootstrap;
  return 0;
}

void datatype::name_i2c(int ind, char *name) {
  if (ind == dim_complex) {
    sprintf(name, "dim_complex");
    return;
  }
  if (ind == dim_mass) {
    sprintf(name, "dim_mass");
    return;
  }
  if (ind == dim_smear) {
    sprintf(name, "dim_smear");
    return;
  }
  if (ind == dim_conf) {
    sprintf(name, "dim_conf");
    return;
  }
  if (ind == dim_jackknife) {
    sprintf(name, "dim_jackknife");
    return;
  }
  if (ind == dim_operator) {
    sprintf(name, "dim_operator");
    return;
  }
  if (ind == dim_t) {
    sprintf(name, "dim_t");
  ...
  if (ind == dim_channel) {
    sprintf(name, "dim_channel");
    return;
  }
  if (ind == dim_momentum) {
    sprintf(name, "dim_momentum");
    return;
  }
  if (ind == dim_param) {
    sprintf(name, "dim_param");
    return;
  }
  if (ind == dim_temporary) {
    sprintf(name, "dim_temporary");
    return;
  }
  if (ind == dim_bootstrap) {
    sprintf(name, "dim_bootstrap");
    return;
  }
  sprintf(name, "none");
}
...
void datatype::add_dimension(int name, int size, double *list) {
  ...
  add_dimension(name, size);
  for (int i = 0; i < size; i++)
    dim[icount].indices[i] = (fabs(list[i]) < 1) ? list[i] * 1000000 : list[i];
}
  ...
  for (int i = 0; i < dim[ind1].n_indices; i++)
    for (int j = 0; j < res.dim[ind2].n_indices; j++)
      if (res.dim[ind2].indices[j] == dim[ind1].indices[i]) {
        dest.indices[dest.n_indices] = dim[ind1].indices[i];
        dest.n_indices++;
        break;
      }
  if (dest.n_indices == 0) {
    printf("no common index!\n");
    return -3;
  }
  return 0;
}
...
/*     int datatype::combine_all_head(*datatype res,int size0)
     {
          ...
          type=res[0].type;
          for(int i=0;i<ndim;i++)
          if(dim[i].n_indices>1)dim[i].n_indices*=-1;//only check the dimension
   with nind=1
          int flag=0;
          for(int j=1;j<rank;j++)
          {
           ...
           if(sizex!=size0){printf("the indices can't fulfill the
   matrix..\n");return -3};
           for(int i=0;i<ndim;i++)
...
void general_data_base::save() { xqcd_file_write_once(name, &type, data); }
...
void general_data_base::set_size(int ind_f, int &size1, int &size2) {
  ...
  for (int i = 0; i < ndim; i++) {
    if (i < ind_f)
      size1 *= dim[i].n_indices;
    if (i > ind_f)
      size2 *= dim[i].n_indices;
  }
}

int general_data_base::new_data(general_data_base &res, int name,
                                int new_size) {
  ...
  for (int i = 0; i < tmp.ndim; i++)
    if (i != ind)
      ...
      res.add_dimension(tmp.dim[i].type, new_size);
  res.initialize();
  return ind;
}

int general_data_base::move_ind_head(general_data_base &res, int name,
                                     int move_flag) {
  general_data_base tmp("");
  tmp.type = type;
  int ind = tmp.find_dim(name);
...
general_data_base *general_data_base::set_pointer(general_data_base &res,
                                                  general_data_base &tmp) {
  if (&res == this)
    return &tmp;
  else
...
int general_data_base::move_ind(general_data_base &res, int name,
                                int move_flag) {
  general_data_base tmp("");
    ...
    if (move_flag != 0) // move to outside
      for (int i = 0; i < size1; i++)
        for (int j = 0; j < nset; j++)
          memcpy(res.data + (j * size1 + i) * size2,
                 tmp1->data + (i * nset + j) * size2, size2 * sizeof(double));
    if (move_flag == 0) // move to inside
      for (int i = 0; i < size1; i++)
        for (int j = 0; j < nset; j++)
          for (int k = 0; k < size2; k++)
            res.data[(i * size2 + k) * nset + j] =
                tmp1->data[(i * nset + j) * size2 + k];
  } else
    copy(res);
  return flag;
}
...
int general_data_base::normal() {
  ...
  if (ind_c == 0)
    ...
    move_ind(*this, dim_conf, 1);
    return 0;
  }
  if (ind_j == 0)
    return 1;
  else if (ind_j > 0) {
    move_ind(*this, dim_jackknife, 1);
    return 0;
  }
  return -1;
...
int general_data_base::sum(general_data_base &res, int name) {
  ...
  for (int i = 0; i < ptmp->ndim; i++)
    if (i != ic)
      res.add_dimension(ptmp->dim[i]);
  res.initialize();
  for (int is1 = 0; is1 < size1; is1++)
    for (int ix = 0; ix < nc; ix++)
      for (int is2 = 0; is2 < size2; is2++)
        res.data[is1 * size2 + is2] +=
            ptmp->data[(is1 * nc + ix) * size2 + is2];
  return 0;
}
...
int general_data_base::aver(general_data_base &res, int name, int flag) {
  ...
  for (int i = 0; i < ptmp->ndim; i++)
    if (i == ic && flag == 1)
      res.add_dimension(ptmp->dim[i].type, 2);
    else if (i != ic)
      res.add_dimension(ptmp->dim[i]);
  res.initialize();
  for (int is1 = 0; is1 < size1; is1++)
    for (int ix = 0; ix < nc; ix++)
      for (int is2 = 0; is2 < size2; is2++)
        res.data[(is1 * ns + 0) * size2 + is2] +=
            ptmp->data[(is1 * nc + ix) * size2 + is2] / nc;
  if (flag != 1)
    return 0;
  for (int is1 = 0; is1 < size1; is1++)
    for (int ix = 0; ix < nc; ix++)
      for (int is2 = 0; is2 < size2; is2++)
        res.data[(is1 * 2 + 1) * size2 + is2] +=
            pow(ptmp->data[(is1 * nc + ix) * size2 + is2] -
                    res.data[(is1 * 2 + 0) * size2 + is2],
                2);
  int fac = (name == dim_jackknife) ? ((nc - 1) * (nc - 1)) : 1;
  if (name == dim_bootstrap)
    fac = nc;
  for (int is1 = 0; is1 < size1; is1++)
    for (int is2 = 0; is2 < size2; is2++)
      res.data[(is1 * 2 + 1) * size2 + is2] =
          sqrt(res.data[(is1 * 2 + 1) * size2 + is2] * fac / (nc * (nc - 1)));
  return 0;
}

int general_data_base::nind_cfg() {
  int name = -1;
  if (find_dim(dim_conf) >= 0)
    name = dim_conf;
  else if (find_dim(dim_jackknife) >= 0)
    name = dim_jackknife;
  else if (find_dim(dim_bootstrap) >= 0)
    name = dim_bootstrap;
  if (name == -1) {
      ...
      for (int ix2 = 0; ix2 < bin_size; ix2++)
        for (int is2 = 0; is2 < size2; is2++)
          tmp.data[(is1 * nc_bin + ix) * size2 + is2] +=
              res.data[((is1 * nc_bin + ix) * bin_size + ix2) * size2 + is2] /
              bin_size;
...
int general_data_base::pick(general_data_base &res, int name, int ist,
                            int ied) {
  int ind_f = find_dim(name);
  if (ind_f < 0) {
    print_void_ind(name);
  ...
  if (ied == INF)
    ...
    int it2 = ied;
    if (ied >= dim[ind_f].n_indices)
      it2 = dim[ind_f].n_indices - 1;
    if (ist < 0 || ist > it2)
      return -1;
...
int general_data_base::pick(general_data_base &res, int name, int sizet,
                            int *list, int flag) {
  char file[500];
  name_i2c(name, file);
  ...
  //         printf("set_pointer\n");
  int size1, size2;
  ptmp->set_size(inf, size1, size2);
  for (int i = 0; i < sizet; i++)
    ind[i] = -1;
  for (int i = 0; i < sizet; i++)
    for (int j = 0; j < ptmp->dim[inf].n_indices; j++)
      if (list[i] == ptmp->dim[inf].indices[j]) {
        ind[i] = j;
        break;
      }
  if (ind[sizet - 1] == -1) {
    ...
    for (int i = 0; i < sizet; i++)
      printf("%10d", list[i]);
    printf(" list\n");
    for (int i = 0; i < ptmp->dim[inf].n_indices; i++)
      printf("%10d", ptmp->dim[inf].indices[i]);
    printf(" ptmp\n");
    for (int i = 0; i < sizet; i++)
      printf("%10d", ind[i]);
    printf(" ind\n");
  ...
  res.clear_ind();
  if (flag == 1)
    res.add_dimension(ptmp->dim[inf].type, sizet, list);
  for (int i = 0; i < ptmp->ndim; i++)
    if (i != inf)
      res.add_dimension(ptmp->dim[i]);
    else if (flag != 1)
  ...
  if (flag == 1)
    for (int i = 0; i < sizet; i++)
      for (int j = 0; j < size1; j++)
        memcpy(res.data + size2 * (i * size1 + j),
               ptmp->data + size2 * (j * ptmp->dim[inf].n_indices + ind[i]),
               size2 * sizeof(double));
  else
    for (int j = 0; j < size1; j++)
      for (int i = 0; i < sizet; i++)
        memcpy(res.data + size2 * (j * sizet + i),
               ptmp->data + size2 * (j * ptmp->dim[inf].n_indices + ind[i]),
               size2 * sizeof(double));
  delete[] ind;
  return 0;
...
int general_data_base::exclude(general_data_base &res, int name, int index) {
  ...
  for (int i = 0; i < size1; i++)
    for (int j = 0; j < ptmp->dim[ind].n_indices; j++)
      if (j != off) {
        memcpy(p, ptmp->data + (i * ptmp->dim[ind].n_indices + j) * size2,
               size2 * sizeof(double));
        p += size2;
      }
  return 0;
...
int general_data_base::folding(general_data_base &res, int *foldfac, int nt2,
                               int its) {
  ...
  for (int it = 0; it < nt; it++) {
    ...
    for (int i = 0; i < inds[1]; i++)
      for (int j = 0; j < inds[2]; j++)
        res.data[(i * nte + itp) * inds[2] + j] +=
            ptmp->data[(i * nt + it) * inds[2] + j] / ne * foldfac[it];
  }
  fflush(stdout);
  return 0;
}

int general_data_base::eff_mass(general_data_base &res, int fold_type, int nt2,
                                int it_st) {
  int inds[3];
  ...
  for (int i = 0; i < inds[1]; i++)
    for (int it = 1; it < nt; it++)
      for (int j = 0; j < inds[2]; j++) {
        ...
        res.data[(i * nt + it) * inds[2] + j] =
            effective_mass(nt2, it0, data[(i * nt + it - 1) * inds[2] + j],
                           data[(i * nt + it) * inds[2] + j], fold_type);
      }
  return 0;
}

int general_data_base::combine(general_data_base &file1,
                               general_data_base &file2, bool sort) {
  if (file1.data == NULL) {
    file2.copy(*this);
    return 1;
  }
  ...
  if (index == file1.ndim) {
    ...
    size1 = 1;
    size2 = file2.size / file2.dim[0].n_indices;
    index = 0;
  } else {
  ...
  for (int i = 0; i < nd2; i++)
    dim[index].indices[i + nd1] = file2.dim[index].indices[i];
  initialize();
  ...
  for (int i = 0; i < size1; i++)
    for (int j = 0; j < nd1 + nd2; j++)
      if (map[j] < nd1)
        memcpy(data + size2 * ((nd1 + nd2) * i + j),
               file1.data + size2 * (nd1 * i + map[j]), sizeof(double) * size2);
      else
        memcpy(data + size2 * ((nd1 + nd2) * i + j),
               file2.data + size2 * (nd2 * i + map[j] - nd1),
               sizeof(double) * size2);
  delete[] map;
  return 0;
}
  ...
  for (int i = 0; i < dim[ind].n_indices; i++)
    for (int j = i + 1; j < dim[ind].n_indices; j++)
      if (dim[ind].indices[i] > dim[ind].indices[j]) {
        ...
        map[j] = t;
        t = dim[ind].indices[i];
        dim[ind].indices[i] = dim[ind].indices[j];
        dim[ind].indices[j] = t;
      }
  return 0;
}

int general_data_base::bootstrap_sample(general_data_base &res,
                                        const int_rand &ran) {
  int ic = find_dim(dim_conf);
  if (ic < 0) {
    print_void_ind(dim_conf);
  ...
  if (&res == this) {
    printf("You can't overwrite the original data with bootstrap sample\n");
    return 1;
  }
  ...
  for (int i = 0; i < size1; i++)
    for (int j = 0; j < ncfg; j++)
      for (int k = 0; k < size2; k++)
        res.data[(i * ncfg + j) * size2 + k] =
            data[(i * ncfg + r[j]) * size2 + k];
  return 0;
}

int general_data_base::bootstrap(general_data_base &res, int nboot,
                                 const int_rand &r) {
  int ic = find_dim(dim_conf);
  if (ic < 0) {
    print_void_ind(dim_conf);
...
/*     void general_data_base::move_inds(general_data_base &res,int
   move_flag,int ncount,...)
     {
        va_list pn;
        va_start(pn,n);
        int *ind[]
...
void data_iterator::copy_data() {
  ...
  for (i = 0; i < nrunningdims; i++) {
    sprintf(localbuf, "_%s%d", xqcd_type_dim_desc[runningdims[i][0]],
            runningdims[i][1]);
  ...
  for (i = nrunningdims - 1; i >= 0; i--) {
    for (j = 0; j < rawdata.ndim; j++)
      if (rawdata.dim[j].type == runningdims[i][0])
        break;
    if (rawdata.dim[j].indices[rawdata.dim[j].n_indices - 1] ==
        runningdims[i][1]) {
...
int check_dim(one_dim &a, one_dim &b) {
  ...
  if (a.n_indices == b.n_indices) {
    ...
    if (ic == 0)
      return 0;
    if (ic == a.n_indices)
      return 1; // supposing all the index are in ordering.
    // if not those cases
    return -1;
  } else {
    for (int m = 0; m < a.n_indices; m++)
      for (int n = 0; n < b.n_indices; n++)
        if (a.indices[m] == b.indices[n])
          return -2;
    return 1;
  }
}
...
int check_index(datatype &a, datatype &b) {
    ...
    if (a.dim[k].type == b.dim[k].type) {
      ...
      if (ind == -1) {
        ...
        for (int i = 0; i < a.dim[k].n_indices; i++)
          printf("%5d", a.dim[k].indices[i]);
        printf("\n");
        for (int i = 0; i < b.dim[k].n_indices; i++)
          printf("%5d", b.dim[k].indices[i]);
        printf("\n");
        return -1;
      }
      if (ind == -2) {
      ...
      if (ind == 1)
        index = k;
      if (icount == 2) {
        printf("more than one dimension is different, exit\n");
        return -1;
      }
    } else {
      //         printf("type of dimension %4d is not match (%6d,%6d),
      // exit\n",k,
  ...
  return index;
  // return the index with different indices. return a.ndim if the datetype is
  // totally the same
}
