...
int get_endian() {
  ...
  if ((*(pc)) == 1) {
    ...
    printf("Warning : The program is only tested on x86-64 environment and may "
           "have issues on other machines : the xqcdio data file is assumed to "
           "be in LITTLE_ENDIAN.\n");
    return BIG_ENDIAN;
  }
}
...
int xqcd_file_write_once(char *name, filetype *type, double *pdata) {
  ...
  if (get_endian() == LITTLE_ENDIAN) {
    ...
    pbufint = make_endian_switch_buffer_int((int *)type,
                                            sizeof(filetype) / sizeof(int));
...
filehandle xqcd_file_open_write_mpi_milc(char *name, filetype *type,
  ...
  if (this_node == 0) {
    ...
    if (get_endian() == LITTLE_ENDIAN)
      iret =
          MPI_File_write_at(handle->fp_mpi, offset, &handle->type,
                            sizeof(filetype) / sizeof(int), MPI_INT, &status);
    else {
      // TODO: make a type for endian switch.
      printf("endian switch not implemented.\n");
      return NULL;
  ...
  for (i = 0; i < handle->type.head.n_dimensions; i++)
    if (handle->type.head.dimensions[i].type == dim_x ||
        handle->type.head.dimensions[i].type == dim_y ||
        handle->type.head.dimensions[i].type == dim_z ||
        handle->type.head.dimensions[i].type == dim_t)
      count = 1;
    else
  ...
  FORALLSITES(dispmem[nmemfile], psSite) {
    ...
    if (flag == 15 &&
        is_on_grid(pgrid, psSite->x, psSite->y, psSite->z, psSite->t)) {
      // debug
      //			printf("node#%d: nmemfile=%d, (%d,%d,%d,%d),
      // dispfile=%d, dispmem=%d.\n", this_node, nmemfile, psSite->x, psSite->y,
      // psSite->z, psSite->t, dispfile[nmemfile], dispmem[nmemfile]);
  ...
  MPI_Type_indexed(nmemfile, ones, dispmem, handle->basetype,
                   &handle->memdatatype);
  MPI_Type_commit(&handle->memdatatype);
  MPI_Type_indexed(nmemfile, ones, dispfile, handle->basetype,
                   &handle->filedatatype);
...
int xqcd_file_write_mpi_milc_begin(filehandle file, double *vec, int *rundims) {
  ...
  MPI_File_set_view(file->fp_mpi, disp, file->basetype, file->filedatatype,
...
void xqcd_type_print(filetype *type) {
  ...
  for (i = 0; i < type->head.n_dimensions; i++) {
    printf("Dimension #%d : type %s : %d indices :\n", i,
           xqcd_type_dim_desc[type->head.dimensions[i].type],
           type->head.dimensions[i].n_indices);
...
int is_on_grid(gridtype *pgrid, int x, int y, int z, int t) {
  if (((x - pgrid->x0) % pgrid->dx) || ((y - pgrid->y0) % pgrid->dy) ||
      ((z - pgrid->z0) % pgrid->dz) || ((t - pgrid->t0) % pgrid->dt))
    return 0;
...
int type_index2disp(filetype *type, int ind[][2]) {
  ...
  for (i = 0; i < type->head.n_dimensions; i++) {
    ...
    for (j = 0;
         j < N_MAX_DIMENSIONS && ind[j][0] != type->head.dimensions[i].type;
         j++)
      ;
    ...
    if (j < N_MAX_DIMENSIONS) {
      ...
      if (k < type->head.dimensions[i].n_indices)
        ...
        printf("Error finding index %d for dimension #%d=%s in function "
               "type_index2disp() .\n",
...
void type_disp2index(filetype *type, int ind[][2], int disp) {
  ...
  for (i = type->head.n_dimensions - 1; i >= 0; i--) {
    ...
    ind[i][1] = type->head.dimensions[i]
                    .indices[disp % type->head.dimensions[i].n_indices];
    disp = (disp - (disp % type->head.dimensions[i].n_indices)) /
           type->head.dimensions[i].n_indices;
  }
