...
namespace Chroma
{
	namespace DerivQuarkDisplacementEnv
	{
		namespace {
			...
			QuarkDisplacement<LatticePropagator>* mesRhoxNablaEDisplace(XMLReader& xml_in,
		...
		template<>
			void
			MesRhoxNablaEDisplace<LatticePropagator>::operator()(LatticePropagator& tmp,
		...
		namespace IHEP
		{
			...
			bool registerAll() 
			{
				...
				if (! registered)
				{
					...
					success &= Chroma::ThePropDisplacementFactory::Instance().registerObject(std::string("RHOxNABLA_E-DERIV"),
							mesRhoxNablaEDisplace);
					registered = true;
				}
				return success;
			}
