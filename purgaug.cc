...
void read(XMLReader &xml_in, const std::string &path, HBGauge &p) {
  try {
    ...
    QDPIO::cerr << "Caught Exception while reading gauge action: " << s
                << std::endl;
    QDP_abort(1);
  }
...
MCControl newMCHeader(const HBItrParams &update_params,
                      const MCControl &mc_control, unsigned long update_no) {
  START_CODE();
...
// Specialise
void saveState(const HBItrParams &update_params, MCControl &mc_control,
...
//--------------------------------------------------------------------------
void doMeas(XMLWriter &xml_out, multi1d<LatticeColorMatrix> &u,
            HBControl &hb_control, bool warm_up_p, unsigned long cur_update,
  ...
  //
  MCControl mc_new =
      newMCHeader(hb_control.hbitr_params, hb_control.mc_control, cur_update);
  ...
  if (!warm_up_p) {
    QDPIO::cout << "Doing " << user_measurements.size() << " user measurements"
                << std::endl;
...
//--------------------------------------------------------------------------
void
doWarmUp(XMLWriter &xml_out, multi1d<LatticeColorMatrix> &u,
         const LinearGaugeAction &S_g, HBControl &hb_control,
         const multi1d<Handle<AbsInlineMeasurement> > &default_measurements,
         const multi1d<Handle<AbsInlineMeasurement> > &user_measurements) {
  START_CODE();
  Real s_plaq;
  ...
  QDPIO::cout << "WarmUp Control: About to do " << to_do << " updates"
              << std::endl;
...
//--------------------------------------------------------------------------
void doProd(XMLWriter &xml_out, multi1d<LatticeColorMatrix> &u,
            const LinearGaugeAction &S_g, HBControl &hb_control,
            const multi1d<Handle<AbsInlineMeasurement> > &default_measurements,
            const multi1d<Handle<AbsInlineMeasurement> > &user_measurements) {
  START_CODE();
  ...
  if (total_updates >
      hb_control.mc_control.n_updates_this_run + cur_update + 1) {
  ...
  for (int i = 0; i < to_do; i++) {
    ...
    QDPIO::cout << "Doing Update: " << cur_update << " warm_up_p = " << false
                << std::endl;
    ...
    // Do measurements
    doMeas(xml_out, u, hb_control, false, cur_update, default_measurements,
           user_measurements);

    // Save if needed
    if (cur_update % hb_control.mc_control.save_interval == 0) {
      saveState(hb_control.hbitr_params, hb_control.mc_control, cur_update,
...
//--------------------------------------------------------------------------
void doHB(multi1d<LatticeColorMatrix> &u, const LinearGaugeAction &S_g,
  ...
  try {
    ...
    if (hb_control.mc_control.n_warm_up_updates > 0) {
      doWarmUp(xml_out, u, S_g, hb_control, default_measurements,
               user_measurements);
      hb_control.mc_control.n_warm_up_updates = 0; // reset
    }

    // Do the production updates
    doProd(xml_out, u, S_g, hb_control, default_measurements,
           user_measurements);
  }
  catch (const std::string &e) {
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    QDPIO::cerr << "Caught standard library exception: " << e.what()
                << std::endl;
    QDP_abort(1);
  }
  catch (...) {
  ...
  try {
    ...
    GaugeAction<P, Q> *gaugeact = TheGaugeActFactory::Instance().createObject(
        gaugeact_string, gaugeact_reader, "/GaugeAction");
    ...
    QDPIO::cerr << "PURGAUG: Caught standard library exception: " << e.what()
                << std::endl;
    QDP_abort(1);
  }
  catch (...) {
    // This might happen on any node, so report it
    std::cerr << "PURGAUG: caught generic exception during measurement"
              << std::endl;
    QDP_abort(1);
  }
  ...
  try {
    ...
    QDPIO::cerr << "Caugth exception while reading measurements: " << e
                << std::endl << std::flush;
