...
void read(XMLReader &xml, const std::string &path, MCControl &p) {
  ...
  try {
    ...
    bool parioP =
        Layout::isIOGridDefined() && (Layout::numIONodeGrid() > 1); // Default
    ...
    if (parioP) {
      QDPIO::cout << "Setting parallel write mode for saving configurations"
                  << std::endl;
      p.save_pario = QDPIO_PARALLEL;
    } else {
      QDPIO::cout << "Setting serial write mode for saving configurations"
                  << std::endl;
...
void saveState(const UpdateParams &update_params, MCControl &mc_control,
               unsigned long update_no, const multi1d<LatticeColorMatrix> &u) {
  // Do nothing
}

// Specialise
template <>
void saveState(const HMCTrjParams &update_params, MCControl &mc_control,
               unsigned long update_no, const multi1d<LatticeColorMatrix> &u) {
  START_CODE();
  ...
  // Save the config
  writeGauge(file_xml, restart_data_buffer, u, restart_config_filename.str(),
             p_new.save_volfmt, p_new.save_pario);
...
void doHMC(multi1d<LatticeColorMatrix> &u,
           ...
           MCControl &mc_control, const UpdateParams &update_params,
...

  {
    ...
    QDPIO::cout << "MC Control: About to do " << to_do << " updates"
                << std::endl;
    ...
    for (int i = 0; i < to_do; i++) {
      ...
      QDPIO::cout << "Doing Update: " << cur_update
      ...
      if (mc_control.repro_checkP &&
        ...
        QDPIO::cout
            << "Saving start config and RNG seed for reproducability test"
            << std::endl;
        ...
        QDPIO::cout << "Saving end config and RNG seed for reproducability test"
                    << std::endl;
        ...
        QDPIO::cout << "Restoring start config and RNG for reproducability test"
                    << std::endl;
        ...
        bool pass = checkReproducability(
            gauge_state.getP(), gauge_state.getQ(), rng_seed_end2,
            repro_bkup_end.getP(), repro_bkup_end.getQ(), rng_seed_bkup_end);

        if (!pass) {
          QDPIO::cout << "Reproducability check failed on update " << cur_update
                      << std::endl;
          ...
          QDPIO::cout << "Reproducability check passed on update " << cur_update
                      << std::endl;
          write(xml_out, "ReproCheck", pass);
          write(xml_log, "ReproCheck", pass);
        }
      ...
      QDPIO::cout << "HMC: start inline measurements" << std::endl;
      {
        ...
        QDPIO::cout << "HMC: initial resetting default gauge field"
                    << std::endl;
        ...
        for (int m = 0; m < default_measurements.size(); m++) {
          ...
          QDPIO::cout << "HMC: finished default measurement = " << m
                      << std::endl;
        }
        ...
        for (int m = 0; m < user_measurements.size(); m++) {
          ...
          QDPIO::cout << "HMC: considering user measurement number = " << m
                      << std::endl;
          ...
          if (cur_update % the_meas.getFrequency() == 0) {
            ...
            // Caller writes elem rule
            push(xml_out, "elem");
            QDPIO::cout << "HMC: calling user measurement number = " << m
                        << std::endl;
            the_meas(cur_update, xml_out);
            QDPIO::cout << "HMC: finished user measurement number = " << m
                        << std::endl;
            pop(xml_out);
          }
        }
        ...
        InlineDefaultGaugeField::reset();
        QDPIO::cout << "HMC: finished final resetting default gauge field"
                    << std::endl;
      }
      ...
      if (cur_update % mc_control.save_interval == 0) {
        ...
        swatch.stop();
        QDPIO::cout << "After saving state: time= " << swatch.getTimeInSeconds()
                    << " secs" << std::endl;
      }
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    QDPIO::cerr << "hmc: Caught Exception while reading file: " << e
                << std::endl;
    QDP_abort(1);
  }
  ...
  try {
    ...
    swatch.start();
    {
      ...
      QDPIO::cout << "Gauge initialization: cfg_type = " << mc_control.cfg.id
                  << std::endl;

      Handle<GaugeInit> gaugeInit(TheGaugeInitFactory::Instance().createObject(
          mc_control.cfg.id, cfgtop, mc_control.cfg.path));
    ...
    swatch.stop();
    QDPIO::cout << "Gauge field reunitarization found " << numbad
    ...
    QDPIO::cerr << "hmc: Caught standard library exception: " << e.what()
                << std::endl;
    QDP_abort(1);
  }
  catch (...) {
    QDPIO::cerr << "hmc: caught generic exception during measurement"
                << std::endl;
    QDP_abort(1);
  }
  ...
  try {
    ...
    QDPIO::cerr << "hmc: Caught exception while reading measurements: " << e
                << std::endl << std::flush;

    QDP_abort(1);
  }

  QDPIO::cout << "There are " << the_measurements.size()
              << " user measurements " << std::endl;
  ...
  snoop.stop();
  QDPIO::cout << "HMC: total time = " << snoop.getTimeInSeconds() << " secs"
              << std::endl;
