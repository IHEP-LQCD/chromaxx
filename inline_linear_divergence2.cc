...
namespace Chroma
{
  namespace InlineLinearDivergence2Env
  {
    namespace
    {
      ...
      {
        return new InlineLinearDivergence2(InlineLinearDivergence2Params(xml_in, path));
      }
  ...
  //! Propagator input
  void read(XMLReader &xml, const std::string &path, InlineLinearDivergence2Params::NamedObject_t::Props_t &input)
  {
  ...
  //! Propagator output
  void write(XMLWriter &xml, const std::string &path, const InlineLinearDivergence2Params::NamedObject_t::Props_t &input)
  {
    push(xml, path);
  ...
  //! Propagator input
  void read(XMLReader &xml, const std::string &path, InlineLinearDivergence2Params::NamedObject_t &input)
  {
    XMLReader inputtop(xml, path);
  ...
  //! Propagator output
  void write(XMLWriter &xml, const std::string &path, const InlineLinearDivergence2Params::NamedObject_t &input)
  {
    push(xml, path);
  ...
  // Param stuff
  InlineLinearDivergence2Params::InlineLinearDivergence2Params() { frequency = 0; }

  InlineLinearDivergence2Params::InlineLinearDivergence2Params(XMLReader &xml_in, const std::string &path)
  {
    try
    {
  ...
  void InlineLinearDivergence2Params::writeXML(XMLWriter &xml_out, const std::string &path)
  {
    push(xml_out, path);
  ...
  namespace
  {
    ...
    void readSinkProp(SinkPropContainer_t &s, const std::string &id)
    {
      ...
      {
        QDPIO::cerr << InlineLinearDivergence2Env::name << ": caught dynamic cast error"
                    << std::endl;
        QDP_abort(1);
      }
      catch (const std::string &e)
                                     ...
                                     XMLWriter &xml_out)
  {
    ...
    QDPIO::cout << InlineLinearDivergence2Env::name << ": linear divergence calculation" << std::endl;
    ...
    {
      QDPIO::cerr << InlineLinearDivergence2Env::name << ": std::map call failed: " << e
                  << std::endl;
      QDP_abort(1);
    }
    ...
    const InlineLinearDivergence2Params::NamedObject_t::Props_t named_obj = params.named_obj.sink_pairs[0];
    AllSinkProps_t all_sinks;
    readAllSinks(all_sinks, named_obj);
    multi1d<int> t_srce = all_sinks.sink_prop_1.prop_header.source_header.getTSrce();
    ...
    QDPIO::cout << InlineLinearDivergence2Env::name << ": initialized " << std::endl;
    ...
    const LatticePropagator &sink_prop_1 =
        TheNamedObjMap::Instance().getData<LatticePropagator>(all_sinks.sink_prop_1.sink_prop_1_id);
    const LatticePropagator &sink_prop_2 =
        TheNamedObjMap::Instance().getData<LatticePropagator>(all_sinks.sink_prop_2.sink_prop_1_id);
...
//        F_tmp = F_mu * shift(u[imu], FORWARD, imu);
    ...
    {
      tmpu2 = where(
          (((Layout::latticeCoordinate(3)) - it0) == 0),
          adj(u[imu]), u[imu]);
      F_mu = sink_prop_1;
      icount = 2 * nt / 2;
      for (int iop = 0; iop < nx / 2; iop++)
      {
        if (icount == 2 * nt / 2)
        {
          quark_loop[imu + icount * 4] = LatticeComplex(trace(F_mu * Gamma(ig) * adj(F_mu) * Gamma(ig)));
        }
        else
        {
          quark_loop[imu + icount * 4] = LatticeComplex(trace(F_mu * Gamma(ig) * adj(F) * Gamma(ig)));
        }
        icount++;
    ...
    for (int imu = 0; imu < 4; imu++)
    {
      quark_loop[imu + icount * 4] = LatticeComplex(trace(Gamma(15) * adj(F) * Gamma(15) * Gamma(ig) * F * Gamma(ig)));
      ig = ig * 2;
    }
    icount++;
    ...
    for (int imu = 0; imu < 4; imu++)
    {
      ...
      quark_loop[imu - 1 + icount * 4] = 0.5 * LatticeComplex(trace(Gamma(15) * adj(F) * Gamma(15) *
                                                                    (F_plus - F_minus) * Gamma(ig)));
      ig = ig * 2;
    }
    icount++;

    QDPIO::cout << InlineLinearDivergence2Env::name << "quark loop done" << std::endl;
    ...
    for (int is = 0; is < op_size; is++)
    {
      for (int ig = 0; ig < 4; ig++)
      {
        ...
        if (Layout::primaryNode())
          ...
          {
            result.data[(is * 4 + ig) * 2 * nt + 2 * t] -= toDouble(hsum[t].elem().elem().elem().real());
            result.data[(is * 4 + ig) * 2 * nt + 2 * t + 1] -= toDouble(hsum[t].elem().elem().elem().imag());
          }
      }
    }
    if (Layout::primaryNode())
    ...
    snoop.stop();
    QDPIO::cout << InlineLinearDivergence2Env::name << ": total time = "
                << snoop.getTimeInSeconds()
                << " secs" << std::endl;

    QDPIO::cout << InlineLinearDivergence2Env::name << ": ran successfully" << std::endl;
